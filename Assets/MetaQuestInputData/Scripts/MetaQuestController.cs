...
namespace IRIS.MetaQuest
{
    ...
    public class MetaQuest3Controller : MonoBehaviour
    {

        [SerializeField] private Transform trackingSpace;
        [SerializeField] private Transform rootTrans;
        protected Publisher<MetaQuest3InputData> _publisher;
        // private Subscriber<string> startVibrationSubscriber;
        // private Subscriber<string> stopVibrationSubscriber;

        void Start()
        {
            _publisher = new Publisher<MetaQuest3InputData>("InputData", false);
            // IRXRNetManager _netManager = IRXRNetManager.Instance;
            // startVibrationSubscriber = new Subscriber<string>($"{_netManager.localInfo.name}/StartVibration", StartVibration);
            // stopVibrationSubscriber = new Subscriber<string>($"{_netManager.localInfo.name}/StopVibration", StopVibration);
        }
